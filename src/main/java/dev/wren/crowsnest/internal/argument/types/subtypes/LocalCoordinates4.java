...
public class LocalCoordinates4 implements Coordinates4{
    public static final char PREFIX_LOCAL_COORDINATE = '^';
    private final double left;
    private final double up;
    private final double forwards;
    private final double look;

    public LocalCoordinates4(double pLeft, double pUp, double pForwards, double pLook) {
        this.left = pLeft;
        this.up = pUp;
    ...
    public static LocalCoordinates4 parse(StringReader pReader) throws CommandSyntaxException {
        ...
        if (pReader.canRead() && pReader.peek() == ' ') {
            ...
            if (pReader.canRead() && pReader.peek() == ' ') {
                ...
                if (pReader.canRead() && pReader.peek() == ' ') {
                    pReader.skip();
                    double d3 = readDouble(pReader, i);
                    return new LocalCoordinates4(d0, d1, d2, d3);
                } else {
                    pReader.setCursor(i);
    ...
    private static double readDouble(StringReader pReader, int pStart) throws CommandSyntaxException {
        if (!pReader.canRead()) {
            ...
            throw Vector4dArgument.ERROR_MIXED_TYPE.createWithContext(pReader);
        } else {
            pReader.skip();
    ...
    public boolean equals(Object pOther) {
        if (this == pOther) {
            return true;
        } else if (!(pOther instanceof LocalCoordinates4)) {
            return false;
        } else {
            LocalCoordinates4 localcoordinates = (LocalCoordinates4)pOther;
            return this.left == localcoordinates.left && this.up == localcoordinates.up && this.forwards == localcoordinates.forwards && this.look == localcoordinates.look;
        }
    }

    public int hashCode() {
        return Objects.hash(this.left, this.up, this.forwards, this.look);
    }
}
